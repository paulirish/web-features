...
-   **Modify `computeBaseline()` function:**
    ...
    export function browsers(compat: Compat) { // This will be renamed to getCoreBrowserSet
...
-   **In `toDist()` function in `scripts/dist.ts`:**
    1.  Define the `baselinen` browser set, which includes the core browsers plus Node.js.
        ...
        const baselinenBrowserSet = [...coreBrowserSet, "nodejs"].map(b => compat.browser(b));
        ```
